...
import {
	Canvas, extend, useFrame, useThree,
...
function Swarm({ count, mouse }) {
	// useRef damit kein Rerender?
	const mesh = useRef();
	const light = useRef();
	const { size, viewport } = useThree();
	const aspect = size.width / viewport.width;

	// useMemo damit nur eine Instanz erstellt wird
	const dummy = useMemo( () => new THREE.Object3D(), []);
	// Generate some random positions, speed factors and timings
	// useMemo damit funktion nur nochmal ausgeführt wird, wenn count sich ändert
...
function Dolly() {
	...
	useFrame( ({ clock, camera }) => camera.updateProjectionMatrix(
